...
class CheckInSelect(discord.ui.Select):
    def __init__(self):
        ...
        thursday_cell = get_cell_indexes(spreadsheet=os.getenv('SHEET_SHIFT_SIGNUPS'), sheet='Sign Up', value="Thursday")
        ...
        for column_index in range(thursday_column, len(latest_data[0]), 2):
            for row_index in range(thursday_row, -1, -1):
                ...
                logger.info(f"adding {shift_day}, {shift_time}, {shift_type} to the CheckIn Select Menu")
                options.append(discord.SelectOption(label=f"{shift_day}, {shift_time}, {shift_type}",emoji="🚶",description=""))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(content="Processing your Check-In shift requests...", ephemeral=True)
        ...
        for response in self.values:
            ...
            logging.info(f"Queue request: {requester} is requesting {shift_type} at {shift_day} {shift_time}")
