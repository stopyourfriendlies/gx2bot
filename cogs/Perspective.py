...
logging.basicConfig(
    filename="perspective_check.log", encoding="utf-8", level=logging.INFO
)
...
class Perspective(commands.Cog):
    ...
    async def perspective_check(self):
        ...
        perspective_results = [
            ...
            response_dict["attributeScores"]["IDENTITY_ATTACK"]["summaryScore"][
                "value"
            ],
            ...
            response_dict["attributeScores"]["SEXUALLY_EXPLICIT"]["summaryScore"][
                "value"
            ],
    ...
    async def subscribe(self, ctx):
        ...
        if str(ctx.message.author.id) not in id_list:
            subscribersSheet.append_row(
                [
                    str(ctx.message.author.id),
                    ctx.message.author.name,
                    str(ctx.message.author.discriminator),
                    0.95,
                    0.95,
                    0.95,
                    0.95,
